...
class ScVectorOp(Node, ScNode):
	...
	def error_condition(self):
		return (
				(not self.inputs["Operation"].default_value in ["ADD", "SUB", "MULT", "VECMULT", "CROSS", "DOT", "ANGLE", "PROJ", "REFL", "ROT", "NORM", "ORTHO", "LERP", "SLERP"])
	...
	def post_execute(self):
		...
		elif (self.inputs["Operation"].default_value == 'VECMULT'):
				out["Value"] = Vector(
					(
						self.inputs["X"].default_value[0] * self.inputs["Y"].default_value[0],
						self.inputs["X"].default_value[1] * self.inputs["Y"].default_value[1],
						self.inputs["X"].default_value[2] * self.inputs["Y"].default_value[2]
					)
				)
